...
export default function TargetCompaniesForm() {
  ...
  const [jobStats, setJobStats] = useState<CompanyJobStats[]>([]);
  ...
  const checkJobAvailability = async () => {
    if (!currentUser || companies.length === 0) return;

    setCheckingJobs(true);
    
    try {
      const stats: CompanyJobStats[] = [];
      
      // Check job availability for each target company
      for (const company of companies) {
        if (company.name && company.position) {
          try {
            const response = await fetch(`/api/jobs/search?company=${encodeURIComponent(company.name)}&keywords=${encodeURIComponent(company.position)}&limit=10`, {
              headers: {
            ...
            console.warn(`Failed to check jobs for ${company.name}:`, error);
            stats.push({
              company: company.name,
              position: company.position,
              availableJobs: 0,
  ...
  const getJobStatsForCompany = (companyName: string, position: string) => {
    return jobStats.find(stat => stat.company === companyName && stat.position === position);
  };
    ...
    <div className="bg-white p-6 rounded-lg shadow-card border border-slate-200">
                      ...
                      <div>
                        ...
                        {company.position && (
                          <div className="flex items-center mt-2 gap-2">
                            <span className="text-sm text-slate-500">Target: {company.position}</span>
                            {(() => {
                              const stats = getJobStatsForCompany(company.name, company.position);
                              return stats ? (
      ...
      <div className="flex justify-between items-center">
        <div className="flex gap-3">
          {companies.length > 0 && companies.some(c => c.name && c.position) && (
            <button
              onClick={checkJobAvailability}
              disabled={checkingJobs}
              className="bg-blue-600 hover:bg-blue-700 disabled:bg-slate-400 disabled:cursor-not-allowed text-white font-medium py-3 px-6 rounded-lg transition-colors"
            >
              {checkingJobs ? (
                <div className="flex items-center">
          ...
          {jobStats.length > 0 && (
            <span className="text-sm text-slate-600 flex items-center">
              ...
              Last checked: {jobStats[0].lastChecked.toLocaleDateString()}
            </span>
          )}
        </div>
        <button
